...
- **Prefer the wrapper `node scripts/gitbug.mjs`** — one-turn triage verbs: `board` (whole board, compact ≤140-char lines; `--status/--label/--area/--prio/--sev/--type/--limit`, `--json` = `gitbug.board.v1`), `next` (deterministic tier order `sev:critical > sev:major > prio:high > sev:minor > prio:normal > prio:low > sev:low > sev:cosmetic > untagged`, then oldest, then id), `brief <ref> [--tail N]` (metadata + cross-refs + last N comments, `--json` = `gitbug.brief.v1`), `apply <ref>` (one locked call: `--label` add → `--label-rm` → `--comment` → `--status`; stops on first failure, `--dry-run` prints the steps); plus `list`/`show`/`resolve`/`new`/`comment`/`close`/`open`/`labels` (`--json`, `--dry-run`). It enforces non-interactive calls, validates labels, serializes invocations, and never trusts the selection fallback. Raw `git bug` only for verbs it lacks (`title edit`, `comment edit`, `user`, `rm`, `bridge`, `push`/`pull`, full-text search).
- **Serialization:** the wrapper queues concurrent invocations behind its own advisory lock (`--lock-timeout`/`GITBUG_LOCK_TIMEOUT`, default 120 s; stale locks are reclaimed automatically), so the old one-command-at-a-time/retry workaround is retired. Raw `git bug` calls made outside the wrapper are not serialized.
...
- **Durable issues/backlog** → **git-bug** (single source; §9). Drive it via `node scripts/gitbug.mjs` — one-turn triage with `board` (filters `--area/--prio/--sev/--type/--label`, `--json`), `next`, `brief`, `apply`; `list`/`show`/`resolve`/`new`/`comment`/`close` for the rest; never keep a TODO file.
