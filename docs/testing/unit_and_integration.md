...
The Agentic Sandbox Studio unit and integration test suites run under the native Node.js test runner (`node --test`) using strict assertions (`node:assert/strict`). The test suite comprises **51 unit test suites** and **46 integration test suites**, executing with zero mocks for core application classes.
