...
- `RealmTemplateBundle` is the authored bundle seam (a format-v1 or format-v2 template plus bundle file bodies): host/pipeline injection may supply either format, the store normalizes once for exposure/launch, and the content pipeline can supply real bundle files without restructuring the launch flow
...
export interface RealmLaunchFromTemplateOptions {
    ...
    // Warning: (ae-forgotten-export) The symbol "RealmInputValues" needs to be exported by the entry point index.svelte.d.ts
...
- **`allowVersionMismatch`** — Explicit confirmation that a payload pinning a different template version may attach: without it a version mismatch fails closed with `ERR_HYDRATION_VERSION_MISMATCH`; with it the mismatch is reported as a receipt warning and the launch proceeds.
...
- **`inputs`** — Operator-assembled supplied input values keyed by declared input id, in the shape-tagged format-v2 form (`{ shape: 'text', text }` or `{ shape: 'files', files }`). The explicit launch/review value wins per key over an attached payload's value; supplying the same input id in both `inputs` and the legacy `inputValues` fails closed (one value per input).
- **`inputValues`** — Legacy launch-level text input values keyed by declared input id (one value per input, shared by every referencing agent). Keys must name declared `text`-shape inputs and values must be strings; an explicit empty string stays launch-sourced (it blocks a `required` input instead of falling back to the declared default). A key naming a `files`-shape input fails closed with a message directing the caller to `inputs`/`payload`.
- **`name`** — Display name override; the template name is used when absent.
- **`package`** — Legacy alias of RealmLaunchFromTemplateOptions.payload: the instance content a hydrator produced. Supplying both `payload` and `package` fails closed.
- **`payload`** — Optional instance payload: a format-v2 payload object (an envelope carrying `formatVersion: 2`, `templateId`, `templateVersion`, and `inputs`) or a legacy format-v1 hydration package, validated against the effective template contract before any Realm record exists. Payload inputs are the base and `inputs`/`inputValues` win per key; the payload's pinned template version is compared against the effective authored template version (a mismatch fails closed unless `allowVersionMismatch` explicitly confirms it).
...
- **`seed`** — Whether to apply the template's resolved placements and directives after every member launches. Defaults to `true`; `false` skips both entirely (launcher toggle) so the operator can seed manually afterwards.
...
One launchable Realm template bundle (the authored seam): the template — format v1 or format v2 — plus the bundle file bodies its `file` prompt parts, placement `file` sources, and input `defaultFile` prefills resolve against.

The bundle is the store's launch seam: host/pipeline injection registers authored bundles through SandboxStoreOptions.realmTemplateBundles, and the store normalizes each template once for exposure and launch (RealmTemplateBundleView). The baked catalog ships the demo bundle with no files; the store freezes the bundle container and a copy of the file map, while the template itself is used as supplied (baked/injected templates are validated at catalog resolution).

#### Members

- **`files`** — Bundle file bodies keyed by bundle-relative path (empty for file-less bundles).
- **`template`** — Authored template the bundle launches (format v1 or format v2).

### `RealmTemplateBundleView` — interface

One effective launch bundle as the launcher and review surfaces see it: the normalized format-v2 template (normalizeTemplate of the authored bundle template) plus the bundle file bodies.

Exposure is normalized so the launcher can read declared inputs, placements, and directives without a format branch; the authored form stays the identity and round-trip source (imports persist/export their authored payload, shipped bundles re-serialize their authored template, and the effective template version is the authored-form pin).

#### Members

- **`files`** — Bundle file bodies keyed by bundle-relative path (the authored file map).
- **`template`** — Normalized format-v2 template (frozen).
